...
class VariationalGaussianMixtureCB():
    ...
    def fit_predict(self, X):
        ...
        for i in range(self.max_iter):
            ...
            resp = self._compute_resp(X, expect)
            self._update_params(X, resp, expect)
            self._m_step(resp)
    ...
    def _compute_expectations(self, D):
        ...
        exp_log_det_T = np.sum(digamma(0.5 * (self.nu - np.arange(0, D)[:,np.newaxis])), axis=0) \
    ...
    def _compute_resp(self, X, expect):
        ...
        for n in range(N):
            for k in range(self.K):
                log_rho[n, k] = np.log(self.weights[k] + 10 * np.finfo(self.weights.dtype).eps)\
                    + 0.5 * expect["log_det_T"][k] - 0.5 * np.trace(
